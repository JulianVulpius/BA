...
import pandas as pd
import numpy as np
from scipy.signal import welch # bei weglassen von novverlap = 50% nperseg
...
def main():
    ...
    with st.sidebar:
        ...
        cleaning_lvl = st.selectbox("Reinigungs-Level", CLEANING_LEVELS, index=2)
        ...
        use_youtube = st.checkbox("Youtube-Daten anzeigen", value=False, 
                                  help="Wenn aktiviert, fließen Youtube Daten teilweise in die Statistiken ein.")
    ...
    if st.session_state['master_data'] is not None:
        ...
        with tab_fein:
            ...
            with st.expander("📘 Methodik: Signalverarbeitung & Mathematische Herleitung", expanded=False):
                st.markdown("### Vom Rohsignal zum kognitiven Score")
                st.markdown(
                    "Die Transformation der EEG-Daten folgt einer strengen mathematischen Pipeline. "
                    "Ziel ist es, die zeitabhängige Spannungsschwankung $x(t)$ in eine frequenzabhängige Leistungsverteilung $P(f)$ zu überführen."
                )

                # --- SCHRITT 1: WELCH METHODE ---
                st.markdown("---")
                st.subheader("1. Transformation (Zeit- zu Frequenzbereich)")
                
                st.markdown(
                    """
                    Das EEG-Rohsignal ist eine **Zeitreihe** (Amplitude über Zeit). Um zu verstehen, *welche* Rhythmen (Alpha, Beta) aktiv sind, 
                    müssen wir das Signal in seine Frequenzanteile zerlegen. Wir nutzen dazu die **Welch-Methode**.
                    """
                )

                c_welch1, c_welch2 = st.columns([1.5, 1])
                with c_welch1:
                    st.markdown("**Ablauf der Welch-Methode:**")
                    st.markdown(
                        """
                        1.  **Segmentierung (Windowing):** Das Signal wird in kleine, überlappende Zeitfenster geschnitten (z.B. 1 Sekunde).
                        2.  **Fourier-Transformation (FFT):** Jedes Fenster wird einzeln transformiert.
                        3.  **Mittelung:** Die Ergebnisse aller Fenster werden gemittelt, um Rauschen zu reduzieren ("Glättung").
                        """
                    )
                with c_welch2:
                    st.info(
                        "**Warum Welch?**\n\n"
                        "Ein einfaches FFT über das ganze Signal wäre zu 'verrauscht' (hohe Varianz). "
                        "Durch das Mitteln vieler kleiner Fenster erhalten wir eine stabile Schätzung der Power Spectral Density (PSD)."
                    )
                st.markdown("---")
                st.markdown("#### Exkurs: Warum überlappende Fenster (Overlap)?")
                
                col_overlap1, col_overlap2 = st.columns([1.5, 1])
                
                with col_overlap1:
                    st.markdown(
                        """
                        Der Code nutzt die Welch-Methode standardmäßig einen **50% Overlap**. 
                        Das bedeutet, die Zeitfenster liegen wie **Dachziegel** übereinander.
                        
                        **Warum ist das nötig?**
                        1.  **Das Fenster-Problem:** Um Rechenfehler an den Kanten zu vermeiden ("Spectral Leakage"), 
                            blendet die Mathematik das Signal am Anfang und Ende jedes Fensters auf Null aus (Glockenkurve / Hanning-Fenster). 
                            Informationen an den Rändern würden also verloren gehen.
                        2.  **Die Lösung:** Durch das Überlappen liegt das, was im ersten Fenster am "dunklen Rand" war, 
                            im nächsten Fenster genau in der "hellen Mitte".
                        """
                    )
                    st.markdown(""" **Spectral Leakage:** *ist eine Verzerrung in der Frequenzanalyse (wie der FFT), bei der Energie einer einzelnen Frequenz in benachbarte Frequenzbereiche „austritt“, 
                        wodurch ein scharfer Peak verschmiert erscheint oder falsche Peaks entstehen. 
                        Dies geschieht, weil reale Signale endlich und innerhalb des Abtastfensters oft nicht perfekt periodisch sind, was zu Diskontinuitäten führt, die höhere Frequenzen in die Analyse einbringen.* """)
                
                with col_overlap2:
                    st.info(
                        "**Code Parameter:**\n"
                        "Fensterlänge = 1 Sekunde\n"
                        "Überlappung = 0,5 Sekunden (Default)\n\n"
                        "Dadurch wird jeder Datenpunkt doppelt verwertet und der Score wird stabiler."
                    )
                
                st.markdown("---")
                st.markdown("#### Wie wird die Power der Frequenzen berechnet?")
                st.markdown(
                    "Wie finden wir heraus ob und wie stark z.B. **10 Hz** im Signal enthalten sind? "
                    "Mithilfe von Korrelation von **10 Hz sin & cos 'Schablonen'**."
                )

                c_math1, c_math2 = st.columns(2)
                with c_math1:
                    st.markdown("**A. Die Zerlegung (Sinus & Kosinus)**")
                    st.markdown(
                        "Da wir nicht wissen, wann genau eine Welle startet (Phase), vergleichen wir das Signal mit zwei um 90° verschobenen Schablonen:"
                    )
                    st.latex(r"A = \sum (Signal \cdot \sin(2\pi f t))")
                    st.latex(r"B = \sum (Signal \cdot \cos(2\pi f t))")
                    st.caption(
                        """Wir multiplizieren das Signal Punkt für Punkt mit der Schablone. 
                        Wenn das Signal passt, wird das Ergebnis deshalb sehr groß, weil hier Plus auf Plus (ergibt Plus) und Minus auf Minus (ergibt ebenfalls Plus) trifft. 
                        Da sich Wellenberg und Wellental synchron bewegen, summieren wir am Ende lauter positive Zahlen auf, statt dass sich Plus und Minus gegenseitig aufheben."""
                    )

                with c_math2:
                    st.markdown("**B. Die Zusammenführung (Pythagoras)**")
                    st.markdown(
                        "Da Sinus und Kosinus orthogonal (senkrecht) zueinander stehen, bilden $A$ und $B$ ein rechtwinkliges Dreieck."
                    )
                    st.latex(r"Hypotenuse = \sqrt{A^2 + B^2}")
                    st.markdown("Diese Hypotenuse entspricht der **Amplitude** (Höhe) der Welle im EEG.")

                # --- DIE FRAGE NACH DER POWER ---
                st.info(
                    "**Warum reicht die Länge (Hypotenuse) für die Power?**\n\n"
                    "Hier greift ein physikalisches Gesetz: **Energie (Power) ist proportional zum Quadrat der Amplitude.**\n\n"
                    "* Stell man sich die Hypotenuse als Seil vor. Die **Länge dieses Seils** ist die exakte Höhe der Welle im EEG.\n"
                    "* Langes Seil = Hohe Welle = Viel neuronale Aktivität.\n"
                    "* Kurzes Seil = Flache Welle = Wenig Aktivität.\n\n"
                )

                c_math3, c_math4 = st.columns(2)
                with c_math3:
                    st.markdown("**C. Berechnung der Power**")
                    st.markdown(
                    "Um die **Power** zu erhalten, quadrieren wir diese Länge einfach wieder (Pythagoras ohne Wurzel):\n\n"
                    )
                    st.latex(r"P(f) = A^2 + B^2")

                # --- SCHRITT 2: BAND INTEGRATION ---
                st.markdown("---")
                st.subheader("2. Extraktion des Frequenzbereichs")
                
                st.markdown(
                    "Das Spektrum liegt nun in kleinen, diskreten Abschnitten vor – den sogenannten **Frequenz-Bins**. "
                    "Wir greifen uns nun die Bins heraus, die zu unserem Band gehören (z.B. **Alpha: 8–13 Hz**)."
                )

                # Zwei Spalten für den Vergleich der Methoden
                c_int1, c_int2 = st.columns([1, 1])
                
                with c_int1:
                    st.markdown("##### Standard (Physik)")
                    st.markdown("Die *Summe* der Energie (Total Power). Abhängig von der Bandbreite.")
                    st.latex(r"P_{Band-Total} = \sum_{i=1}^{N} P_i")

                with c_int2:
                    st.markdown("##### Unser Ansatz (Data Science)")
                    st.markdown("Der *Durchschnitt* (Mean Density). Vergleichbar über Bänder hinweg.")
                    st.latex(r"P_{Band-Mean} = \frac{1}{N} \sum_{i=1}^{N} P_i")

                # Definition der Variablen (Legende)
                st.markdown(
                    """
                    > **Mathematische Legende:** > $P_i$: Die Power eines einzelnen **Bins** (ein Frequenz-Punkt, z.B. bei 10 Hz).  
                    > $N$: Die Anzahl der Bins im Frequenzband (die "Breite" des Bandes).
                    """
                )

                # --- WARUM? (Die Logik) ---
                st.info(
                    "**Warum `np.mean` statt Summe?**\n\n"
                    "Wir wollen Ratios bilden (z.B. Alpha / Beta), aber die Bänder sind unterschiedlich breit:\n"
                    "* **Alpha** (8-13 Hz) ist schmal (wenige Bins $N$).\n"
                    "* **Beta** (13-30 Hz) ist breit (viele Bins $N$).\n\n"
                    "Würden wir die **Summe** nutzen, gewinnt Beta fast immer, nur weil es mehr Bins aufaddiert. "
                    "Durch den **Mittelwert** (`np.mean`) teilen wir durch $N$. So vergleichen wir die reine **Intensität** der Bänder, unabhängig von ihrer Breite."
                )

                # --- SCHRITT 3: RÄUMLICHE MITTELUNG ---
                st.markdown("---")
                st.subheader("3. Räumliche Mittelung (Spatial Averaging)")
                st.markdown(
                    "Das EEG liefert Werte für 8 Kanäle. Um einen **globalen Zustand** zu erhalten, bilden wir den Durchschnitt über alle Elektroden."
                )
                st.latex(r"P_{Global} = \frac{1}{8} \sum_{i=1}^{8} P_{Kanal_i}")

                # --- SCHRITT 4: RATIO ---
                st.markdown("---")
                st.subheader("4. Der finale Ratio-Score (Verhältnis)")
                st.markdown(
                    "Absolute Werte sind schwer vergleichbar. Wir nutzen daher Verhältnisse, z.B. **Alpha/Beta**."
                )
                st.latex(r"Alpha/Beta  = \frac{P_{Global}(\alpha)}{P_{Global}(\beta)}")
            ...
            with st.expander("📘 Methodik: Normalisierung & Threshold", expanded=False):
                ...
                st.markdown(
                    ...
                    * $\mu$: Der persönliche Durchschnittswert des Probanden, in der Song Aufnahme Session.
                    * $\sigma$: Die persönliche Standardabweichung (Schwankungsbreite) des Probanden, in der Song Aufnahme Session.
                    """
                )
                ...
                with c_dec1:
                    ...
                    st.markdown(
                        rf"Nutzt die Original-Werte (µV/Ratios) und passt dementsprechend den Threshold dynamisch an."
                    )
                    st.markdown("✅ Intuitiv verständlich beim Vergleich der Analyse zwischen Songs eines Probanden innerhalb einer Session.")
                    st.markdown("❌ Nicht gut für Vergleiche zwischen Probanden.")

                with c_dec2:
                    st.markdown("**C) Z-Score (Normalisierung)**")
                    st.markdown("*Einsatz: Probanden-Vergleiche*")
                    st.markdown(
                        "Transformiert Daten auf eine Einheitsskala. Der Threshold ist fix (z.B. 0.5), da die Daten genormt sind."
                    )
                    st.markdown("✅ Macht Probanden Messwerte vergleichbar.")
                    st.markdown("❌ Abstraktere Werte.")

                st.markdown("##### Zusammenfassend:")
                st.markdown(
                    "Um den Song eines Probanden einzuordnen, berechnen **Relativ** und **Z-Score** beide den Mittelwert des **Song-Kontexts** (alle Songs einer Playlist + Baseline der Session) und damit die Standardabweichung des **Song-Kontexts** (also definiert der Song Kontext auch den Probanden). Nur der Umgang damit unterscheidet sich:"
                )
                st.markdown("- **Relativ:** Passt den Threshold an den Daten des Probanden an (Threshold = Faktor(Schwellenwert) $\\times$ Song-Kontext $\\sigma$).")
                st.markdown("- **Z-Score:** Passt die Daten des Probanden an den Threshold an (Daten = Rohwert $/$ Song-Kontext $\\sigma$).")

                st.markdown("---")
                st.markdown("### Warum bleiben die Ergebnisse (Spearman & Kendall & Genre-Ranganalyse) global identisch?")
                st.markdown(
                    "Da wir nun wissen, dass beide Methoden auf der gleichen Standardabweichung basieren, erklärt sich, warum die Korrelationen ($r_s$, $\\tau$) exakt gleich bleiben:"
                )
                
                c_why1, c_why2 = st.columns(2)
                
                with c_why1:
                    st.markdown("**1. Das mathematische Gleichgewicht**")
                    st.markdown(
                        r"Es ist mathematisch egal, ob wir den Toleranz-Bereich skalieren (Relativ) oder die Daten normalisieren (Z-Score). "
                        r"Die Entscheidung, wie zwei Songs (vom EEG) geranked werden, bleibt identisch:"
                    )
                    st.latex(r"|A - B| < T \cdot \sigma \iff \frac{|A - B|}{\sigma} < T")
                    st.caption("Tool-Tabelle darüber wie stark der maximale Threshold benötigt wird um das Ranking zu beeinflussen.")

                with c_why2:
                    st.markdown("**2. Korrelation als 'Meta-Normalisierung'**")
                    st.markdown(
                        "Der Korrelationskoeffizient selbst fungiert hier als der ultimative Gleichmacher zwischen den Probanden. "
                        "Er abstrahiert von den absoluten Werten und bewertet nur die **Struktur der Ordnung**."
                    )
                    st.markdown(
                        "Daher ist es für den Durchschnittswert aller Probanden irrelevant, welche Berechnungsmethode gewählt wurde, solange die **Rangreihenfolge** (siehe Punkt 1) erhalten bleibt."
                    )
            ...
            with st.expander("📘 Methodik: Rangzuordnung", expanded=False):
                ...
                st.markdown(
                    "**Wir wollen also prüfen:** Stimmt die **subjektive Reihenfolge** mit der **objektiven Reihenfolge** überein, wenn wir den Probanden eine reallistische Toleranz zugestehen. (Da wir nicht so feingranular unterscheiden können wie das EEG.)"
                )
            ...
            with st.expander("📘 Methodik: Spearman vs. Kendall", expanded=False):
                ...
                tab_spear, tab_kend, tab_eval, tab_pearson = st.tabs(["Spearman (Distanz)", "Kendall (Paare)", "Bewertung (Output)", "Exkurs: Pearson & Linearität"])
                ...
                # --- TAB 2: KENDALL ---
                    ...
                    # --- TAB 3: INTERPRETATION ---
                        ...
                        # --- PEARSON ---
                        with tab_pearson:
                            st.header("Exkurs: Die mathematische Identität")
                            
                            st.info(
                                "**Die Kern-These:**\n"
                                "Spearman ist keine 'andere' Art der Korrelation. Spearman ist lediglich die Anwendung der "
                                "**Pearson-Produkt-Moment-Korrelation** auf Ränge."
                            )
                            
                            st.markdown("### 1. Das Problem: Linearität vs. Monotonie")
                            
                            c_p1, c_p2 = st.columns(2)
                            with c_p1:
                                st.markdown("**Pearson ($r$) auf Rohdaten**")
                                st.caption("Erwartet eine Gerade")
                                st.markdown(
                                    "Pearson misst **lineare** Zusammenhänge. "
                                    "Er fragt: *\"Wenn die Spannung um 1µV steigt, steigt das Ranking exakt um X Plätze?\"*"
                                )
                                st.latex(r"y = a \cdot x + b")
                                st.error("Problem: Das Gehirn arbeitet nicht linear. Der Unterschied zwischen Platz 1 und 2 fühlt sich anders an als zwischen 9 und 10.")
                            
                            with c_p2:
                                st.markdown("**Spearman ($r_s$) auf Rängen**")
                                st.caption("Erwartet Monotonie")
                                st.markdown(
                                    "Durch das Umwandeln in Ränge (Ranking) linearisieren wir das Problem. "
                                    "Uns ist egal, *wie stark* die Spannung steigt, solange das Ranking mitzieht."
                                )
                                st.latex(r"x_i < x_j \Rightarrow y_i < y_j")
                                st.success("Lösung: Wir rechnen den Pearson-Koeffizienten nicht auf den krummen Messwerten, sondern auf den sauberen Rängen.")

                            st.markdown("---")
                            st.markdown("### 2. Die Herleitung: Von Pearson zur vereinfachten Formel")
                            st.markdown(
                                "Oft wird Spearman mit dieser einfachen Formel gelehrt: $1 - \\frac{6 \\sum d^2}{n(n^2 - 1)}$. "
                                "Doch wo kommt die her? Sie ist eine **mathematische Abkürzung**, die nur unter perfekten Bedingungen gilt."
                            )
                            
                            st.markdown("**Schritt A: Die wahre Formel (Pearson)**")
                            st.markdown("Der Korrelationskoeffizient ist definiert als Kovarianz geteilt durch das Produkt der Standardabweichungen:")
                            st.latex(r"r = \frac{\sum (x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum (x_i - \bar{x})^2 \sum (y_i - \bar{y})^2}}")
                            
                            st.markdown("**Schritt B: Der Spezialfall (Keine Ties)**")
                            st.markdown(
                                "Wenn wir Ränge von $1$ bis $n$ haben (z.B. 1, 2, 3, 4, 5) und **keine doppelten Werte** vorkommen, wissen wir bereits vorher, "
                                "wie groß Mittelwert und Varianz sind (da es immer die gleichen Zahlen sind)."
                            )
                            st.latex(r"\bar{x} = \bar{y} = \frac{n+1}{2}")
                            st.latex(r"\sum (x_i - \bar{x})^2 = \frac{n(n^2 - 1)}{12}")
                            
                            st.markdown(
                                "Setzt man diese feststehenden Terme in die Pearson-Formel ein und vereinfacht den Bruch, "
                                "**entsteht automatisch die bekannte Spearman-Formel:**"
                            )
                            st.latex(r"\text{Pearson Formel} \xrightarrow{\text{keine Ties}} 1 - \frac{6 \sum d^2}{n(n^2 - 1)}")
                            
                            st.markdown("---")
                            st.markdown("### 3. Warum 'Fair' (Tied Ranks) die Rückkehr zu Pearson erfordert")
                            
                            st.warning(
                                "⚠️ **Das Problem:** In unserer Analyse 'Fair' erzeugen wir **doppelte Ränge** (Ties). "
                                "Wenn Song A und Song B im EEG fast gleich sind, erhalten beide den Rang **1.5**."
                            )
                            
                            c_tie1, c_tie2 = st.columns(2)
                            
                            with c_tie1:
                                st.markdown("**Die vereinfachte Formel scheitert**")
                                st.markdown(
                                    "Die Annahme $\\sum (x_i - \\bar{x})^2 = \\frac{n(n^2 - 1)}{12}$ stimmt nicht mehr, "
                                    "da die Varianz der Ränge sinkt (1.5 liegt näher am Mittelwert als 1 oder 2)."
                                )
                                st.markdown("$\\to$ Die Formel liefert **falsche Werte**.")
                                
                            with c_tie2:
                                st.markdown("**Die Lösung: Back to the Roots**")
                                st.markdown(
                                    "Da Spearman per Definition *Pearson auf Rängen* ist, ignorieren wir die vereinfachte Formel "
                                    "und füttern die **ursprüngliche Pearson-Formel (Schritt A)** mit unseren 'krummen' Rängen (1.5, 3, 4.5)."
                                )
                                st.markdown("$\\to$ Das Ergebnis ist mathematisch exakt.")

                            st.success(
                                "**Fazit für die Arbeit:**\n"
                                "Die Methode 'Fair' ist kein Hack. Sie nutzt die Robustheit der Pearson-Korrelation, "
                                "um physiologische Unschärfe (Ties) statistisch korrekt abzubilden. "
                                "Der Code verwendet daher im Hintergrund immer die volle Kovarianz-Berechnung, nicht die Näherungsformel."
                            )
            ...
            is_zscore_active = "Z-Score" in calc_method
            if not is_zscore_active:
                st.warning(
                    "💡 **Wichtiger Hinweis zur Vergleichbarkeit:**\n\n"
                    "1. Für einen Vergleich zwischen Probanden bei **Signifikante Einzelfehler (Details)** "
                    "sollte in den Einstellungen oben zwingend der **Z-Score** gewählt werden, da absolute Werte (µV) individuell stark schwanken.\n"
                    "2. Die untenstehende **Gesamtübersicht** wurde **automatisch auf Z-Score (Threshold 0.5) normalisiert**, "
                    "um eine statistisch valide Durchschnittsberechnung zu gewährleisten. Für die Anpassungen des Thresholds muss **C)** Toleranz-Logik gewählt werden."
                )
            ...
            with st.expander("📘 Methodik: Berechnung des 'Weighted Impact Score'", expanded=False):
                ...
                st.markdown(
                    ...
                    2.  **Einfluss auf den Score (Gatekeeper):** Selbst wenn noch eine Rang-Differenz besteht, prüft der Threshold hier in Schritt 3 erneut, ob die Signal-Differenz signifikant genug für eine Bestrafung ist. [Auch bei Standard!]
            ...
            weighted_cases = [] # Detail-Ansicht (folgt User-Einstellung)
            weighted_cases_forced_z = [] # Gesamtübersicht (immer Z-Score mit Treshold 0,5)
            ...
            if not analysis_df.empty:
                for (pid, playlist), group in analysis_df.groupby(['participant', 'playlist']):
                    ...
                    for idx, row in temp_grp.iterrows():
                        ...
                        w_score_z = 0.0
                        if is_zscore_active:
                            # Wenn Z-Score eh schon aktiv ist, nehmen wir einfach den Wert von oben
                            w_score_z = weighted_score
                        else:
                            # 1. Z-Werte berechnen
                            if np.std(raw_vals) == 0: vals_z = np.zeros(len(raw_vals))
                            else: vals_z = (raw_vals - np.mean(raw_vals)) / np.std(raw_vals)
                            thresh_z = 0.5 # Fixer Threshold
                            
                            # 2. Rang Berechnung für Z-Werte (Logik kopieren)
                            if "Standard" in impact_mode:
                                calc_ranks_z = calculate_physio_ranks(vals_z, 0.0)
                            elif "Sehr Fair" in impact_mode:
                                # gleichen Algo wie oben
                                calc_ranks_z = np.zeros(len(vals_z))
                                sort_idx_z = np.argsort(vals_z)[::-1]
                                sorted_values_z = vals_z[sort_idx_z]
                                current_rank_counter_z = 1
                                i_idx_z = 0
                                while i_idx_z < len(vals_z):
                                    cluster_indices_z = [sort_idx_z[i_idx_z]]
                                    j_z = i_idx_z + 1
                                    while j_z < len(vals_z) and abs(sorted_values_z[i_idx_z] - sorted_values_z[j_z]) < thresh_z:
                                        cluster_indices_z.append(sort_idx_z[j_z]); j_z += 1
                                    available_ranks_z = list(range(current_rank_counter_z, current_rank_counter_z + len(cluster_indices_z)))
                                    cluster_subj_ranks_z = subj_ranks[cluster_indices_z]
                                    sorted_rel_indices_z = np.argsort(cluster_subj_ranks_z)
                                    for k_idx_z, rel_idx_z in enumerate(sorted_rel_indices_z):
                                        calc_ranks_z[cluster_indices_z[rel_idx_z]] = available_ranks_z[k_idx_z]
                                    current_rank_counter_z += len(cluster_indices_z); i_idx_z = j_z
                            else: # Fair
                                calc_ranks_z = calculate_physio_ranks(vals_z, thresh_z)

                            # 3. Score Berechnung Z
                            # Wir greifen auf die aktuellen Indizes zu (idx ist der row-index der Gruppe)
                            c_rank_z = calc_ranks_z[idx] # idx kommt aus deiner iterrows Schleife "for idx, row in temp_grp.iterrows():"
                            
                            # Referenzwert bestimmen (auf Z-Basis)
                            sorted_vals_desc_z = np.sort(vals_z)[::-1]
                            ref_idx_z = min(s_rank - 1, len(sorted_vals_desc_z) - 1)
                            val_ref_z = sorted_vals_desc_z[ref_idx_z]
                            
                            rank_diff_z = abs(s_rank - c_rank_z)
                            val_diff_z = abs(vals_z[idx] - val_ref_z)
                            
                            if val_diff_z >= thresh_z:
                                w_score_z = rank_diff_z * val_diff_z
                        
                        # Speichern für die Gesamtübersicht
                        weighted_cases_forced_z.append({
                            'Song': row['item'], 
                            'Playlist': row['playlist'], 
                            'Weighted Score': w_score_z
                        })
            ...
            if weighted_cases:
                ...
                st.markdown("**Gesamtübersicht (Ø Weighted Score aller Songs mithilfe von Z-Score):**")

                if weighted_cases_forced_z:
                    w_df_z = pd.DataFrame(weighted_cases_forced_z)
                    stats_weighted = generate_detailed_stats(w_df_z, 'Weighted Score', agg_func='mean')
        ...
        with tab_global:
            ...
            for m in METRICS:
                ...
                # B) WEIGHTED SCORES
                is_native_z = "Z-Score" in calc_method
                if m == METRICS[0]: # Nur einmal anzeigen, nicht für jede Metrik wiederholen
                    if not is_native_z:
                        st.warning("💡 **Berechnung:** Daten wurden für die globale Vergleichbarkeit automatisch auf **Z-Score** normalisiert (Fixer Threshold: **0.5**). Für die Anpassungen des Thresholds muss **C)** Toleranz-Logik gewählt werden.")

                for (pid, playlist), group in analysis_df.groupby(['participant', 'playlist']):
                    raw_vals = group[m].values
                    subj_ranks = group['subj_rank'].values
                    
                    # --- 1. DATEN & THRESHOLD BESTIMMEN ---
                    if is_native_z:
                        # Nimm die Logik und den Threshold aus den Globalen Settings
                        vals_z, effective_thresh = apply_tolerance_logic(raw_vals)
                    else:
                        # Erzwinge Z-Score mit Standard-Threshold 0.5
                        loc_mean = np.mean(raw_vals)
                        loc_std = np.std(raw_vals)
                        if loc_std == 0: 
                            vals_z = np.zeros(len(raw_vals))
                        else:
                            vals_z = (raw_vals - loc_mean) / loc_std
                        
                        effective_thresh = 0.5

                    # --- 2. RANG BERECHNUNG (Auf Basis der Z-Werte) ---
                    if "Standard" in global_ws_mode:
                        # Bei Standard ignorieren wir den Threshold für das Ranking (0.0)
                        calc_ranks = calculate_physio_ranks(vals_z, 0.0) 
                        
                    elif "Sehr Fair" in global_ws_mode:
                        calc_ranks = np.zeros(len(vals_z))
                        sort_idx = np.argsort(vals_z)[::-1]
                        sorted_values = vals_z[sort_idx]
                        
                        current_rank_counter = 1
                        i_idx = 0
                        while i_idx < len(vals_z):
                            cluster_indices = [sort_idx[i_idx]]
                            j = i_idx + 1
                            while j < len(vals_z) and abs(sorted_values[i_idx] - sorted_values[j]) < effective_thresh:
                                cluster_indices.append(sort_idx[j])
                                j += 1
                        ...
                        calc_ranks = calculate_physio_ranks(vals_z, effective_thresh)

                    # --- 3. SCORE BERECHNUNG ---
                    sorted_vals_desc = np.sort(vals_z)[::-1]
                    temp_grp = group.copy().reset_index(drop=True)
                    
                    temp_grp['val_z'] = vals_z
                    temp_grp['calc_rank'] = calc_ranks 
                    ...
                    for idx, row in temp_grp.iterrows():
                        ...
                        # Hier greift der Threshold als Gatekeeper (auch bei Standard!)
                        is_physio_error = val_diff >= effective_thresh
            ...
            with st.expander("📘 Methodik: Statistische Herleitung & Interpretation", expanded=False):

                st.markdown("### Warum Std-Abweichung für die Y-Achse (Individualität)?")
                st.markdown(
                    """
                    Hier herrscht evtl. Verwirrung: *"Müssten wir für den Vergleich zwischen Probanden nicht auch Z-Scores nehmen?"*
                    
                    **Die Antwort:** Wir vergleichen hier nicht die *Höhe* der Entspannung, sondern die **Einigkeit** der Gruppe.
                    Da die Werte bereits als Z-Scores vorliegen (normalisiert sind), nutzen wir die **Standardabweichung (Std)** dieser Z-Scores als Maß für den **Konsens**.
                    
                    * **Niedrige Std (z.B. 0.2):** Alle Punkte liegen eng beieinander. Alle Probanden reagieren fast identisch = **Hoher Konsens (Universalität)**.
                    * **Hohe Std (z.B. 1.5):** Die Punkte streuen weit. Ein Proband reagiert stark positiv (+2), ein anderer stark negativ (-1) = **Hohe Polarisierung (Individualität)**.
                    """
                )

                st.markdown("---")
                st.markdown("### Was ist der Schwellenwert?")
                st.markdown(
                    """
                    Wir verwenden einen fixen Cut-Off von **0.5 Standardabweichungen**. Dieser Wert leitet sich aus den Konventionen für Effektstärken (nach Cohen) und der Normalverteilung ab:
                    
                    * **< 0.5 SD:** Der Effekt liegt noch im Bereich der "natürlichen Streuung" (Rauschen) und wird als schwach/vernachlässigbar betrachtet.
                    * **> 0.5 SD:** Der Effekt hebt sich sichtbar vom Rauschen ab (mittlerer Effekt).
                    * **Farbgebung:** In der Grafik werden daher Punkte erst ab ±0.5 grün/rot eingefärbt, um statistisch irrelevante Minimalschwankungen visuell auszublenden.
                    """
                )

            with st.expander("📘 Methodik: Der Song-Kontext & Berechnung", expanded=False):
                st.markdown("### 1. Definition: Was ist der 'Song-Kontext'?")
                st.info(

                    "Man kann bei den Messwerte einer Person, einen Song in Playlist 1 (Session 1) nicht direkt mit dem gleichen Song in Playlist 4 (Session 2) vergleichen."
                )
                st.markdown(r"""
                Deshalb definieren wir für jeden Song einen **lokalen Kontext ($C_{Playlist}$)**. 
                Dieser besteht aus allen Datenpunkten der **aktuellen Playlist, in der der Song abgespielt wurde** (inkl. der Baseline dieser Session).
                
                * **Mathematische Konsequenz:** Der Mittelwert ($\mu$) und die Standardabweichung ($\sigma$) zur Berechnung des Z-Scores werden für jede Playlist individuell bestimmt.
                * **Ergebnis:** Der Z-Score drückt aus, wie stark sich ein Song innerhalb seines spezifischen Umfelds abhebt.
                """)

                st.markdown("---")
                st.markdown("### 2. Variablen-Legende")
                st.markdown(
                    r"""
                    | Symbol | Bezeichnung | Erklärung |
                    | :--- | :--- | :--- |
                    | $x_{i,C}$ | Rohwert | Messwert von Song $i$ in dem lokalen Kontext $C_{Playlist}$. |
                    | $\mu_C, \sigma_C$ | **Playlist-Profil** | Mittelwert und Std-Abw. aller Songs innerhalb des lokalen Kontext $C$. |
                    | $Z_{i,C}$ | **Kontext-Z-Score** | Den Z-Score für Song $i$ im lokalen Kontext $C$. |
                    | $\Omega_{Genre}$ | Genre-Pool | Die Menge *aller* Kontext-Z-Scores aller Songs, die zu diesem Genre gehören. |
                    | $X_{Genre}$ | **Effektivität** | Durchschnittliche Wirkung des Genres relativ zu den jeweiligen Song-Kontexten. |
                    | $Y_{Genre}$ | **Individualität** | Standardabweichung der Genre Wirkung (Maß für Polarisierung/Uneinigkeit). |
                    """
                )

                st.markdown("---")
                st.markdown("### 3. Schritt-für-Schritt Berechnung")

                c_f1, c_f2, c_f3 = st.columns(3)
                
                with c_f1:
                    st.markdown("**Schritt A: Lokale Normalisierung**")
                    st.caption("Ebene: Song im Playlist-Kontext")
                    st.latex(r"Z_{i,C} = \frac{x_{i,C} - \mu_C}{\sigma_C}")

                with c_f2:
                    st.markdown("**Schritt B: Effektivität (X)**")
                    st.caption("Ebene: Gesamtes Genre")
                    st.latex(r"X_{Genre} = \frac{1}{N} \sum Z_{i,C \in \Omega_{Genre}}")
                    st.markdown("Aggregation aller normalisierten Werte für dieses Genre über alle Probanden.")
                    
                with c_f3:
                    st.markdown("**Schritt C: Polarisierung (Y)**")
                    st.caption("Ebene: Gesamtes Genre")
                    st.latex(r"Y_{Genre} = \sqrt{\frac{\sum (Z_i - X_{Genre})^2}{N}}")
                    st.markdown("Messung der Streuung innerhalb des Genre-Pools. Hohes Y bedeutet hohe individuelle Unterschiede.")

            z_data_list = []
            
            # WICHTIG: Gruppierung nach Participant UND Playlist für Song-Kontext
            for (pid, playlist), group in genre_df.groupby(['participant', 'playlist']):
                
                raw_vals = group[target_metric_genre].values
                
                # Lokaler Kontext für Z-Score Berechnung
                local_mean = np.mean(raw_vals)
                local_std = np.std(raw_vals)
                
                if local_std == 0: 
                    z_scores = np.zeros(len(raw_vals))
                else:
                    z_scores = (raw_vals - local_mean) / local_std
        ...
        with tab_verlauf:
            ...
            if not learning_stats_s:
                ...
                with tab_s:
                    ...
                    with c2:
                        # Session 2 Wert als Hauptzahl, Differenz als Delta-Indikator
                        avg_s2 = df_perf_s[col_s2].mean()
                        avg_delta = df_perf_s[col_delta].mean()
                        st.metric(
                            label=f"Ø Genauigkeit S2 ({calc_mode_9})", 
                            value=f"{avg_s2:.3f}", 
                            delta=f"{avg_delta:+.3f}" # Das erzeugt den kleinen grünen/roten Pfeil
                        )
                ...
                with tab_k:
                    ...
                    with c2:
                        # Session 2 Wert als Hauptzahl, Differenz als Delta-Indikator
                        avg_k2 = df_perf_k[col_s2].mean()
                        avg_k_delta = df_perf_k[col_delta].mean()
                        st.metric(
                            label=f"Ø Genauigkeit S2 ({calc_mode_9})", 
                            value=f"{avg_k2:.3f}", 
                            delta=f"{avg_k_delta:+.3f}"
                        )

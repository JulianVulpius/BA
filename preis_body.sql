...
PROCEDURE export_ausschreibung_to_excel(
    ...
    v_gaeb_version VARCHAR2(20);
BEGIN
    -- 1. PRE-CHECK: Welche GAEB Version ist das?
    BEGIN
        SELECT xt.version INTO v_gaeb_version
        FROM pd_import_x86 x,
             XMLTABLE(
    ...
    -- 2. PRE-CHECK: Finden wir überhaupt Positionen (Items)? (Tiefen/Namespace-Unabhängig!)
    SELECT count(*) INTO v_item_count
    FROM pd_import_x86 x,
         XMLTABLE(
            '//*[local-name()="Item"]' 
    ...
    -- Wenn keine Positionen gefunden wurden, werfen wir eine saubere Fehlermeldung
    IF v_item_count = 0 THEN
        raise_application_error(-20001, 
            'Fehler beim Einlesen: Es wurden keine Leistungspositionen gefunden. ' ||
            'Die Datei meldet sich als GAEB Version "' || v_gaeb_version || '". ' ||
...
PROCEDURE export_ausschreibung_to_excel_unified(
    ...
    p_regionen varchar2, p_liferant varchar2, p_user_id number
       ...
       v_vergabesumme1 number; -- Vergabesumme Von
       v_vergabesumme2 number; -- Vergabesumme Bis
       v_trimm number;
       l_trimm number;
...
BEGIN
    ...
    BEGIN
        SELECT name INTO v_original_filename 
        FROM pd_import_x86 
        WHERE id = p_blob_id;
    ...
    IF v_trimm IS NOT NULL THEN
        v_col_offset := 2; -- Wenn Parameter vorhanden, verschiebe alle späteren Spalten um 2
    END IF;
    ...
    -- Spaltenbreiten (Dynamisch angepasst)
    xlsx_writer.col_width(workbook, sheet_1, 1, 40);
    xlsx_writer.col_width(workbook, sheet_1, 2, 20);
    xlsx_writer.col_width(workbook, sheet_1, 3, 20);
    xlsx_writer.col_width(workbook, sheet_1, 4, 20);
    xlsx_writer.col_width(workbook, sheet_1, 5, 20);
    ...
    xlsx_writer.col_width(workbook, sheet_1, 12 + v_col_offset, 20);
    xlsx_writer.col_width(workbook, sheet_1, 13 + v_col_offset, 20);
    xlsx_writer.col_width(workbook, sheet_1, 14 + v_col_offset, 20);

    -- Header Labels (Statische Basis-Spalten)
    ...
    -- Dynamische Header einfügen
    IF v_trimm IS NOT NULL THEN
        xlsx_writer.add_cell(workbook, sheet_1, 1, 12, style_id => cs_master, text => 'GETRIMMTER MW');
        xlsx_writer.add_cell(workbook, sheet_1, 1, 13, style_id => cs_master, text => 'GESAMT GETRIMMTER MW');
    END IF;

    -- Verschiebbare Header
    xlsx_writer.add_cell(workbook, sheet_1, 1, 12 + v_col_offset, style_id => cs_master, text => 'MAX PREIS');
    xlsx_writer.add_cell(workbook, sheet_1, 1, 13 + v_col_offset, style_id => cs_master, text => 'GESAMT MAX PREIS');
    xlsx_writer.add_cell(workbook, sheet_1, 1, 14 + v_col_offset, style_id => cs_master, text => 'ANZAHL VERGABEN');

    -- SCHLEIFE ÜBER ALLE ITEMS (NAMESPACE & TIEFEN UNABHÄNGIG)
    FOR i IN (
        SELECT 
        ...
        FROM pd_import_x86 x,
                ...
                'for $i in //*[local-name()="Item"]
                 return <row>
                          ...
                          <master_code>{data($i/ancestor::*[local-name()="BoQCtgy"][last()]/@RNoPart)}</master_code>
                          <master_name>{normalize-space($i/ancestor::*[local-name()="BoQCtgy"][last()]/*[local-name()="LblTx"])}</master_name>
                          <parent_code>{data($i/ancestor::*[local-name()="BoQCtgy"][1]/@RNoPart)}</parent_code>
                          <parent_name>{normalize-space($i/ancestor::*[local-name()="BoQCtgy"][1]/*[local-name()="LblTx"])}</parent_name>
                        </row>'
                PASSING XMLType(x.DATEI, nls_charset_id('AL32UTF8'))
                COLUMNS
    ...
    LOOP
       BEGIN
            ...
            -- Getrimmten Mittelwert sicher & nativ pro Zeile berechnen
            IF v_trimm IS NOT NULL THEN
                SELECT avg(case 
       ...
       -- GETRIMMTER_PREIS (l_trimm) in Insert aufgenommen
       ...
       VALUES
            (p_blob_id, l_id, i.name, i.code, i.me, to_number(replace(nvl(i.menge,0),'.',',')), l_min, l_avg, l_max, l_median, l_trimm, i.parent, i.master, i.parent_name, i.master_name, i.kennung, l_count, CASE WHEN i.file_= 'x82' THEN 1 ELSE 0 END, to_number(replace(nvl(i.einheitspreis,0),'.',',')), to_number(replace(nvl(i.einheitspreis,0),'.',','))*to_number(replace(nvl(i.menge,0),'.',',')));

    END LOOP;

    -- Erzeugen der Auswertungsansicht in Excel
    ...
    loop
       ...
       -- Zellen für getrimmten Mittelwert bedingt ausgeben
       IF v_trimm IS NOT NULL THEN
           xlsx_writer.add_cell(workbook, sheet_1, l_row, 12,style_id => j.number_format, value_ => j.getrimmter_preis);
           xlsx_writer.add_cell(workbook, sheet_1, l_row, 13,style_id => j.number_format, value_ => j.gesamt_trimm);
       END IF;

       -- Evtl. Verschiebte Ausgaben durch v_col_offset
       xlsx_writer.add_cell(workbook, sheet_1, l_row, 12 + v_col_offset,style_id => j.number_format, value_ => j.max_preis);
       xlsx_writer.add_cell(workbook, sheet_1, l_row, 13 + v_col_offset,style_id => j.number_format, value_ => j.gesamt_max);
       xlsx_writer.add_cell(workbook, sheet_1, l_row, 14 + v_col_offset,style_id => j.cs_style, value_ => j.code_count);
    ...
    -- Mailversand der Auswertung mit dynamischem Dateinamen
...
procedure auswertung_to_excel_2(p_von date,
                                ...
                                p_regionen varchar2,
                                p_liferant varchar2,
                                p_trimm number,    -- NEU: Eingabeparameter für den Trimm-Faktor in Prozent
                                p_user_id number) as
       ...
       c_y_split constant integer := 14; -- Header Zeile 14, Daten ab 15
       ...
       type t_muster_rec is record(
            ...
            einheit           varchar2(50),
            menge_soll        number,       -- Menge aus SQL (1)
            minimum_preis     number,
            mittelwert_preis  number,
            median_preis      number,
            getrimmter_preis  number,       -- NEU: Feld für den getrimmten Preis
       ...
       -- Variablen für Formeln/Dropdown
       v_dropdown_list varchar2(4000) := '"Mittelwert,Median,getrimmter MW"';
       l_col_dropdown  varchar2(5) := 'E';
       l_col_val_ep    varchar2(5) := 'F';
       l_col_menge     varchar2(5) := 'C';
       l_col_mean      varchar2(5) := 'I';
       l_col_median    varchar2(5) := 'J';
       l_col_trimm     varchar2(5) := 'K';
       l_col_max       varchar2(5) := 'L';
       l_col_count     varchar2(5) := 'M';
...
begin
    ...
    -- Brauner Hintergrund & Brauner Rahmen für Legende
    ...
    -- Spezielle Styles nur für die Legende (damit die Tabelle selbst nicht braun wird)
    cs_legende_header := xlsx_writer.add_cell_style(workbook, font_id => font_db_bold_white, fill_id => fill_braun, border_id => border_braun, vertical_alignment => 'center');
    cs_leg_rot := xlsx_writer.add_cell_style(workbook, fill_id => fill_rot, vertical_alignment => 'center', font_id => font_db, border_id => border_braun);
    cs_leg_gelb := xlsx_writer.add_cell_style(workbook, fill_id => fill_gelb, vertical_alignment => 'center', font_id => font_db, border_id => border_braun);
    cs_leg_hellgelb := xlsx_writer.add_cell_style(workbook, fill_id => fill_hellgelb, vertical_alignment => 'center', font_id => font_db, border_id => border_braun);
    cs_leg_hellgruen := xlsx_writer.add_cell_style(workbook, fill_id => fill_hellgruen, vertical_alignment => 'center', font_id => font_db, border_id => border_braun);
    cs_leg_gruen := xlsx_writer.add_cell_style(workbook, fill_id => fill_gruen, vertical_alignment => 'center', font_id => font_db, border_id => border_braun);
    ...
    cs_rot := xlsx_writer.add_cell_style(workbook, fill_id => fill_rot, vertical_alignment => 'center', font_id => font_db);
    cs_gelb := xlsx_writer.add_cell_style(workbook, fill_id => fill_gelb, vertical_alignment => 'center', font_id => font_db);
    cs_hellgelb := xlsx_writer.add_cell_style(workbook, fill_id => fill_hellgelb, vertical_alignment => 'center', font_id => font_db);
    cs_hellgruen := xlsx_writer.add_cell_style(workbook, fill_id => fill_hellgruen, vertical_alignment => 'center', font_id => font_db);
    cs_gruen := xlsx_writer.add_cell_style(workbook, fill_id => fill_gruen, vertical_alignment => 'center', font_id => font_db);
    ...
    -- HEADER (Zeile 14)
    -- ==========================================
    xlsx_writer.add_cell(workbook, sheet_1, 14, 1, style_id => cs_center_bold_grey, text => 'Pos.');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 2, style_id => cs_center_bold_grey, text => 'Pos.-Text');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 3, style_id => cs_center_bold_grey, text => 'Menge');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 4, style_id => cs_center_bold_grey, text => 'Einheit');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 5, style_id => cs_center_bold_grey, text => 'Validierungspreis');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 6, style_id => cs_center_bold_grey, text => 'Val. EP');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 7, style_id => cs_center_bold_grey, text => 'Val. GP');

    xlsx_writer.col_width(workbook, sheet_1, 1, 40);
    xlsx_writer.col_width(workbook, sheet_1, 2, 50);
    xlsx_writer.col_width(workbook, sheet_1, 3, 15);
    xlsx_writer.col_width(workbook, sheet_1, 4, 10);
    xlsx_writer.col_width(workbook, sheet_1, 5, 25);
    ...
    xlsx_writer.col_width(workbook, sheet_1, 10, 15);
    xlsx_writer.col_width(workbook, sheet_1, 11, 17);
    xlsx_writer.col_width(workbook, sheet_1, 12, 15);
    xlsx_writer.col_width(workbook, sheet_1, 13, 25); 


    -- Daten laden (MIT statischer 1 für menge_soll)
    ...
    if l_contracts.count > 0 then
        for idx in 1..l_contracts.count loop
            l_col_index := c_x_split + l_column + 10;
    ...
    -- Legende mit braunem Rahmen und weiß-braunem Header
    xlsx_writer.add_cell(workbook, sheet_1, 14, 13, style_id => cs_center_bold_grey, text => l_contract_count || ' Vergaben');
    xlsx_writer.add_cell(workbook, sheet_1, 7, 13, style_id => cs_legende_header, text => 'Legende:');
    xlsx_writer.add_cell(workbook, sheet_1, 8, 13, style_id => cs_leg_rot, text => '= 0');
    xlsx_writer.add_cell(workbook, sheet_1, 9, 13, style_id => cs_leg_gelb, text => '= 1 - 4');
    xlsx_writer.add_cell(workbook, sheet_1, 10, 13, style_id => cs_leg_hellgelb, text => '= 5 - 9');
    xlsx_writer.add_cell(workbook, sheet_1, 11, 13, style_id => cs_leg_hellgruen, text => '= 10 - 24');
    xlsx_writer.add_cell(workbook, sheet_1, 12, 13, style_id => cs_leg_gruen, text => '≥ 25');

    -- Header für die Statistiken
    xlsx_writer.add_cell(workbook, sheet_1, 14,  8, style_id => cs_center_bold_grey, text => 'Minimum');
    xlsx_writer.add_cell(workbook, sheet_1, 14,  9, style_id => cs_center_bold_grey, text => 'Mittelwert');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 10, style_id => cs_center_bold_grey, text => 'Median');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 11, style_id => cs_center_bold_grey, text => 'Getrimmter MW');
    xlsx_writer.add_cell(workbook, sheet_1, 14, 12, style_id => cs_center_bold_grey, text => 'Maximum');
    ...
    select parent_master,
            ...
            pos_text,
            einheit,
            1, -- HIER WURDE DIE 1 EINGEFÜGT für menge_soll
            minimum_preis,
            mittelwert_preis,
            median_preis,
            getrimmter_preis,
    ...
    from (
            with mustern as (
                select  m.id,
                        ...
                        round(max(ap_trimm.trimm_preis), 2) getrimmter_preis, -- NEU: Getrimmter Preis 
                ...
                -- Join für den getrimmten Mittelwert über PERCENT_RANK 
                left join (
                            select umsetzung_code,
                                   avg(case 
                                         when total_count <= 2 then avg_ep -- Verhindert leere Felder bei < 3 Preisen
    ...
    if l_mustern.count > 0 then
        for idx in 1..l_mustern.count loop
            ...
            xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 1, style_id => l_style, text => trim(l_mustern(idx).position));
            xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 2, style_id => l_style, text => trim(l_mustern(idx).pos_text));

            -- Spalte 3: Menge (Wert 1 aus DB-Select wird genutzt)
            if l_mustern(idx).parent_master = 'CHILD' then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 3, style_id => l_style, value_ => l_mustern(idx).menge_soll);
            else
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 3, style_id => l_style, text => null);
            end if;

            xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 4, style_id => l_style, text => trim(l_mustern(idx).einheit));

            -- Spalten 5-9 und Formeln
            if l_mustern(idx).parent_master = 'CHILD' then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 5, style_id => l_style, text => 'Mittelwert');

                -- Formel für Val. EP
                v_formula := 'IF(' || l_col_dropdown || l_row_index || '="Mittelwert",' || l_col_mean || l_row_index || ',' ||
                             'IF(' || l_col_dropdown || l_row_index || '="Median",' || l_col_median || l_row_index || ',' ||
                             'IF(' || l_col_dropdown || l_row_index || '="getrimmter MW",' || l_col_trimm || l_row_index || ',0)))';
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 6, style_id => number_format, formula => v_formula);

                -- Formel für Val. GP (Menge * Val. EP)
                v_formula := l_col_menge || l_row_index || '*' || l_col_val_ep || l_row_index;
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 7, style_id => number_format, formula => v_formula);
            else
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 5, style_id => l_style, text => null);
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 6, style_id => l_style, text => null);
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 7, style_id => l_style, text => null);
            end if;

            -- Statistische Werte befüllen (rutschen auf Spalte 8 bis 12)
            if l_mustern(idx).parent_master = 'PARENT' then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index,  8, style_id => l_style, text => 'Minimum');
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index,  9, style_id => l_style, text => 'Mittelwert');
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 10, style_id => l_style, text => 'Median');
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 11, style_id => l_style, text => 'Getrimmter MW');
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 12, style_id => l_style, text => 'Maximum');
            elsif l_mustern(idx).parent_master = 'MASTER' then
                 xlsx_writer.add_cell(workbook, sheet_1, l_row_index,  8, style_id => l_style, text => '');
                 xlsx_writer.add_cell(workbook, sheet_1, l_row_index,  9, style_id => l_style, text => '');
                 xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 10, style_id => l_style, text => '');
                 xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 11, style_id => l_style, text => '');
                 xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 12, style_id => l_style, text => '');
            else
                if l_mustern(idx).minimum_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 8, style_id => number_format, value_ => l_mustern(idx).minimum_preis); end if;
                if l_mustern(idx).mittelwert_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 9, style_id => number_format, value_ => l_mustern(idx).mittelwert_preis); end if;
                if l_mustern(idx).median_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 10, style_id => number_format, value_ => l_mustern(idx).median_preis); end if;
                if l_mustern(idx).getrimmter_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 11, style_id => number_format, value_ => l_mustern(idx).getrimmter_preis); end if;
                if l_mustern(idx).maximum_preis is not null then xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 12, style_id => number_format, value_ => l_mustern(idx).maximum_preis); end if;
            end if;

            -- Anzahl
            if l_mustern(idx).anzahl is null then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => l_style, text => '');
            elsif l_mustern(idx).anzahl = 0 then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => cs_rot, value_ => l_mustern(idx).anzahl);
            elsif l_mustern(idx).anzahl between 1 and 4 then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => cs_gelb, value_ => l_mustern(idx).anzahl);
            elsif l_mustern(idx).anzahl between 5 and 9 then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => cs_hellgelb, value_ => l_mustern(idx).anzahl);
            elsif l_mustern(idx).anzahl between 10 and 24 then
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => cs_hellgruen, value_ => l_mustern(idx).anzahl);
            else
                xlsx_writer.add_cell(workbook, sheet_1, l_row_index, 13, style_id => cs_gruen, value_ => l_mustern(idx).anzahl);
            end if;
    ...
    if l_mustern.count > 0 then
         xlsx_writer.add_data_validation(
            xlsx => workbook, 
            sheet => sheet_1, 
            sqref => 'E15:E' || (14 + l_mustern.count), 
            formula1 => v_dropdown_list
        );
    end if;

    workbook := print_param_worksheet(workbook, p_von, p_bis, p_regionen, p_lvs, p_liferant, p_trimm);

    xlsx_writer.freeze_sheet(workbook, sheet_1, c_x_split + 10, c_y_split);
...
begin
  ...
  style_bold   := xlsx_writer.add_cell_style(p_ws, font_id => font_db_bold);
      ...
      IF p_id IS NOT NULL THEN
          ...
          IF l_lieferant_raw IS NOT NULL THEN
              BEGIN
                  ...
                  IF l_lieferant_namen IS NOT NULL THEN l_lieferant_namen := l_lieferant_namen || '...'; END IF;
              EXCEPTION WHEN OTHERS THEN l_lieferant_namen := l_lieferant_raw || '...';
              END;
          END IF;
          ...
          IF l_lvs_raw IS NOT NULL THEN
              BEGIN
                  ...
                  IF l_lvs_namen IS NOT NULL THEN l_lvs_namen := l_lvs_namen || '...'; END IF;
              EXCEPTION WHEN OTHERS THEN l_lvs_namen := l_lvs_raw || '...';
              END;
          END IF;
  ...
  xlsx_writer.add_cell(p_ws, v_sheet, 1, 1, style_id => style_bold, text => 'Eingegebene Parameter sind: ');

  FOR i IN 1 .. l_names.COUNT LOOP
    IF l_vals(i) IS NOT NULL THEN
        xlsx_writer.add_cell(p_ws, v_sheet, l_row_num, 1, style_id => style_bold, text => l_names(i) || ':');
        xlsx_writer.add_cell(p_ws, v_sheet, l_row_num, 2, text => l_vals(i));
...
begin
  ...
  IF p_liferant_raw IS NOT NULL THEN
      BEGIN
          ...
          IF l_lieferant_namen IS NOT NULL THEN l_lieferant_namen := l_lieferant_namen || '...'; END IF;
      EXCEPTION WHEN OTHERS THEN l_lieferant_namen := p_liferant_raw || '...';
      END;
  END IF;
  ...
  IF p_lvs_raw IS NOT NULL THEN
      BEGIN
          ...
          IF l_lvs_namen IS NOT NULL THEN l_lvs_namen := l_lvs_namen || '...'; END IF;
      EXCEPTION WHEN OTHERS THEN l_lvs_namen := p_lvs_raw || '...';
      END;
  END IF;
  ...
  IF p_regionen IS NOT NULL THEN
      BEGIN
          ...
          IF l_region_namen IS NOT NULL THEN l_region_namen := l_region_namen || '...'; END IF;
      EXCEPTION WHEN OTHERS THEN l_region_namen := p_regionen || '...';
      END;
  END IF;

  l_names := SYS.ODCIVARCHAR2LIST('Von', 'Bis', 'Region', 'LVS', 'Lieferanten');

  l_vals := SYS.ODCIVARCHAR2LIST(TO_CHAR(p_von, 'DD.MM.YYYY'), TO_CHAR(p_bis, 'DD.MM.YYYY'), l_region_namen, l_lvs_namen, l_lieferant_namen);
  ...
  IF p_trimm IS NOT NULL THEN
      ...
      xlsx_writer.add_cell(p_ws, v_sheet, l_row_num, 2, text => p_trimm || ' %');
      l_row_num := l_row_num + 1;
  END IF;

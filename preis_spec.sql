create or replace PACKAGE PREISDATENBANK_PKG AS
  ...
  PROCEDURE auswertung_to_excel_2(p_von date, p_bis date, p_lvs varchar2,p_regionen varchar2,p_liferant varchar2,p_trimm number default null,p_user_id number);

  procedure auswertung_to_excel_leser(p_von date, p_bis date, p_lvs varchar2,p_regionen varchar2,p_liferant varchar2,p_trimm number default null,p_user_id number);
  ...
  FUNCTION print_param_worksheet(p_ws IN OUT xlsx_writer.book_r, p_von IN DATE, p_bis IN DATE, p_regionen IN VARCHAR2, p_lvs_raw IN VARCHAR2, p_liferant_raw IN VARCHAR2, p_trimm number default null) return xlsx_writer.book_r;
